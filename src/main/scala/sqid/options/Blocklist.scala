...
sealed abstract case class Blocklist(value: Set[String]) {
  ...
  def filter(alphabet: Alphabet): Blocklist =
    Blocklist.apply(value.filter(_.forall(alphabet.value.map(_.toLower).contains(_))))
}

object Blocklist {
  def apply(list: Set[String]) = new Blocklist(
    list.filterNot(_.length < 3).map(_.toLowerCase())
  ) {}
  def empty: Blocklist = apply(Set.empty)
  def default: Blocklist = apply(
    Set(
      "0rgasm",
      "1d10t",
      "1d1ot",
